//! `Spinner`, ported from `component-shell`'s `spinner.rs`.
//!
//! A cycling loading spinner. The constructor takes no arguments; size, icon,
//! color, and easing are recorded methods.

use std::sync::Arc;

use gpui::{
    div, ease_in_out, ease_out_quint, linear, AnyElement, Hsla, IntoElement as _,
    ParentElement as _, Refineable as _, Styled as _,
};
use gpui_component::{spinner::Spinner, try_parse_color, IconName, Sizable as _, Size};
...
impl SpinnerMaterializer {
    fn component<'a>(
        ...
        payload
            ...
            .ok_or_else(|| "Spinner received an incompatible payload".to_string())?;
        Ok(operations
            .into_iter()
...
impl ComponentMaterializer for SpinnerMaterializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> Result<AnyElement, String> {
        ...
        let mut element = div().child(Self::component(request.payload(), operations)?);
...
pub(super) fn register(registry: &mut ComponentRegistry) {
    registry
                    ...
                    MethodDescriptor::new(
                        ...
                        |arguments| match arguments {
                            [ComponentArgument::Enum(size)] => match size.as_str() {
                                "xsmall" => {
                                    Ok(ComponentPayload::new(SpinnerOp::Size(Size::XSmall)))
                                }
                                "small" => Ok(ComponentPayload::new(SpinnerOp::Size(Size::Small))),
                                "medium" => {
                                    Ok(ComponentPayload::new(SpinnerOp::Size(Size::Medium)))
                                }
        ...
        .expect("the built-in Spinner descriptor is valid");
}
...
mod tests {
    ...
    fn spinner_rejects_an_incompatible_payload() {
        let error = SpinnerMaterializer::component(&ComponentPayload::new(()), std::iter::empty())
            .err()
            .unwrap();
        assert_eq!(error, "Spinner received an incompatible payload");
    }
}
