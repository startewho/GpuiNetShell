//! `Icon`, ported from `component-shell`'s `navigation/icon.rs`.
//!
//! An SVG icon loaded from a relative path beneath the application's asset
//! root. Absolute paths and parent traversal are rejected. It accepts no
//! children; `size`, `color`, and `rotate` are recorded methods.

use std::path::{Component, Path};
use std::sync::Arc;

use gpui::{AnyElement, IntoElement as _, Refineable as _, Styled as _};
use gpui_component::{try_parse_color, Icon, Sizable as _, Size};
...
impl ComponentMaterializer for IconMaterializer {
    fn materialize(&self, mut request: MaterializeRequest<'_>) -> Result<AnyElement, String> {
        let path = request
            ...
            .ok_or_else(|| "Icon received an incompatible payload".to_string())?;
        ...
        if request.children_len() != 0 {
            return Err("Icon does not accept children".to_string());
        }
...
pub(super) fn register(registry: &mut ComponentRegistry) {
    registry
        .register(
            ComponentDescriptor::new("Icon", Arc::new(IconMaterializer))
                .with_constructors(vec![ConstructorDescriptor::new(
                    "Icon",
                    vec![ArgumentDescriptor::new("path", ArgumentSchema::String)],
                    |arguments| match arguments {
                        [ComponentArgument::String(path)] => {
                            icon_path(path).map(IconPayload).map(ComponentPayload::new)
                        }
                    ...
                    MethodDescriptor::new(
                        ...
                        |arguments| match arguments {
                            [ComponentArgument::Enum(value)] => {
                                size(value).map(ComponentPayload::new)
                            }
        ...
        .expect("the built-in Icon descriptor is valid");
}
