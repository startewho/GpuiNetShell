...
| 新增 | 多窗口：每个顶层窗口 = 一个独立会话。ABI 加 `open_window(parent, flags) -> session` / `close_window(session)`；Rust `host` 用会话 id 分配器 + 每会话 ingress，`OpenChild` 命令在父窗口任务里 `cx.open_window`，`on_window_closed` 清理会话并回调托管；托管 `GpuiApplication.Session` 拆分每窗口的 view/arena/registry，`OpenWindow(Func<View>) -> WindowHandle`（Run 前入队、首帧 flush）、`WindowHandle.Invalidate/Close`；`ManagedMenuAction` 携带所属会话；`SessionId`/`ChildWindowsUseCustomTitlebar`。Sample 加 Multi-Window 页 + `SecondaryWindowView`，`--open-windows=N` / `--close-after=MS` 验证 | — | 7 | `…6C55` | Multi-Window | ✅ |
